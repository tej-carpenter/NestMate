...
export function HostBookingsClient() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      const { data: dbBookings, error: bookingsError } = await (supabase.from("bookings").select("*").eq("host_id", user.id) as any).order("created_at", { ascending: false });
      ...
      if (enrichedBookings.length > 0) {
        ...
        const [usersRes, listingsRes] = await Promise.all([
          ...
          (supabase.from("listings").select("id, title") as any).in("id", listingIds)
        ]);
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead className="bg-black/5 dark:bg-white/5 text-[color:var(--muted)]">
            <tr>
              <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[12px]">Guest Name</th>
              <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[12px]">Contact</th>
              ...
              <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[12px]">Details</th>
          ...
          <tbody className="divide-y divide-[color:var(--border)]">
            {bookings.map((booking) => (
              <tr key={booking.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                <td className="px-6 py-4 font-medium text-[color:var(--foreground)]">{booking.users?.full_name || "Guest"}</td>
                <td className="px-6 py-4">
                  <p className="text-sm text-[color:var(--foreground)]">{booking.users?.phone || "No phone"}</p>
                  <p className="text-xs text-[color:var(--muted)]">{booking.users?.email || "No email"}</p>
                </td>
                ...
                <td className="px-6 py-4">
                  <p className="text-sm text-[color:var(--foreground)]">Qty: {booking.quantity || 1} &middot; {formatRupee(booking.rent_amount)}</p>
                  <div className="mt-2 space-y-1 border-t border-[color:var(--border)] pt-2">
                    {booking.users?.age && <p className="text-xs text-[color:var(--muted)]">Age: {booking.users.age}</p>}
                    {booking.users?.gender && <p className="text-xs text-[color:var(--muted)] capitalize">Gender: {booking.users.gender}</p>}
                    {booking.users?.government_id && <p className="text-xs text-[color:var(--muted)]">ID: {booking.users.government_id}</p>}
                    {booking.users?.address && <p className="text-xs text-[color:var(--muted)] truncate max-w-[150px]" title={booking.users.address}>Addr: {booking.users.address}</p>}
                  </div>
                </td>
                <td className="px-6 py-4">
            ...
            {bookings.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-[color:var(--muted)]">No bookings found.</td>
              </tr>
            )}
          </tbody>
        </table>
